...
def show_data():
    ...
    for file in matching_files:
        json_list.append(show_csv_to_json(file, avg))
...
def show_csv_to_json(csv_file_path, avg):
    data_list = []
    ...
    with open(os.path.join(FILE_PATH, csv_file_path), mode='r', newline='', encoding='utf-8') as csvfile:
        ...
        data_list =  get_average(csvreader, avg)
...
def download_data():
    ...
    json_list = []
    for file in matching_files:
        json_list.append(download_csv_to_json(file))
...
def download_csv_to_json(csv_file_path):
    data_list = []
    date = get_file_date(csv_file_path)
